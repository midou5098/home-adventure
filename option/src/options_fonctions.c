...
void InitSnow(void) {
    ...
    snowTex = IMG_LoadTexture(renderer, ASSET_OPTIONS_SNOW);
    if (!snowTex) {
...
void InitBackground(void) {
    ...
    barFullTex = IMG_LoadTexture(renderer, ASSET_OPTIONS_BAR_FULL);
    barEmptyTex = IMG_LoadTexture(renderer, ASSET_OPTIONS_BAR_EMPTY);

    volIcons[0] = IMG_LoadTexture(renderer, ASSET_OPTIONS_VOLUME_1);
    volIcons[1] = IMG_LoadTexture(renderer, ASSET_OPTIONS_VOLUME_2);
    volIcons[2] = IMG_LoadTexture(renderer, ASSET_OPTIONS_VOLUME_3);
    volIcons[3] = IMG_LoadTexture(renderer, ASSET_OPTIONS_VOLUME_4);

    sunIcons[0] = IMG_LoadTexture(renderer, ASSET_OPTIONS_SUN_1);
    sunIcons[1] = IMG_LoadTexture(renderer, ASSET_OPTIONS_SUN_2);
    sunIcons[2] = IMG_LoadTexture(renderer, ASSET_OPTIONS_SUN_3);
...
void LoadSettings(void) {
    if (!options_settings_load_from_path("settings.dat", &settings)) {
        SDL_Log("settings.dat read failed, using defaults.");
...
void SaveSettings(void) {
    if (!options_settings_save_to_path("settings.dat", &settings)) {
        SDL_Log("settings.dat write failed.");
    }
}
...
void InitAudio(void) {
    ...
    click_sfx = Mix_LoadWAV(ASSET_OPTIONS_CLICK);
    shoot1 = Mix_LoadWAV(ASSET_OPTIONS_SHOOT);
    shoot2 = Mix_LoadWAV(ASSET_OPTIONS_SHOOT);
    music = Mix_LoadMUS(ASSET_OPTIONS_MUSIC);
...
void InitShooters(void) {
    leftShooter.texture  = IMG_LoadTexture(renderer, ASSET_OPTIONS_THIEF_LEFT);
    rightShooter.texture = IMG_LoadTexture(renderer, ASSET_OPTIONS_THIEF_RIGHT);
    bulletTex = IMG_LoadTexture(renderer, ASSET_OPTIONS_BULLET);
    brickTex  = IMG_LoadTexture(renderer, ASSET_OPTIONS_BRICK);
...
void LoadCreditsFromFile(void) {
    ...
    FILE* f = fopen(ASSET_OPTIONS_CREDITS, "rb");
    if (!f) {
...
static int GetOptionLayout(int index, SDL_Rect* labelRect, SDL_Rect* sliderRect, SDL_Rect* rowRect) {
    ...
    if (index == 6) {
        SDL_Rect buttonRect = GetReturnButtonRect();
        if (labelRect) {
