...
int options_settings_resolve_global_path(char* out_path, size_t out_path_size)
{
    ...
    for (i = 0; i < sizeof(root_candidates) / sizeof(root_candidates[0]); i++) {
        ...
        snprintf(marker_path, sizeof(marker_path), "%s/launcher/main.c", base);
        if (file_exists_local(marker_path)) {
            snprintf(out_path, out_path_size, "%s/global_settings.dat", base);
            return 1;
        }
    }

    snprintf(out_path, out_path_size, "global_settings.dat");
    return 1;
}
