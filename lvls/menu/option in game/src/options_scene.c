...
void options_scene_render(void)
{
...

    {
        ...
        if (render_w <= 0 || render_h <= 0) {
            if (SDL_GetRendererOutputSize(renderer, &render_w, &render_h) != 0 && window) {
        ...
        if (render_w > 0 && render_h > 0) {
            ...
            SDL_Rect backdrop = {0, 0, render_w, render_h};
...
void options_scene_render_global_brightness_overlay(SDL_Renderer* target_renderer)
{
    ...
    SDL_Rect rect = {0, 0, render_w, render_h};
